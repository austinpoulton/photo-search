...
class CaptionLoader(val captionFileName: String = "/Flickr8k.token.txt") {
  ...
  def loadCaptions():Map[String, ImageDocument] = {
    ...
    val captionTupleList = captionStrs.map(s => Caption(s)).filter(copt => copt == Some).map(c => (c.get.photoId, c.get.captionId, c.get))
    ...
    val photoDocMap = captionTupleList.foldLeft(new HashMap[String, ImageDocument]) { (imgDocMap, capTuple) =>
      if (imgDocMap(capTuple._1) == None) imgDocMap + (capTuple._1 -> ImageDocument(capTuple._1, capTuple._3))
